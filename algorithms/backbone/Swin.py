...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=(6, 10), shift_size=(0, 0),
...
class FinalPatchExpand_X4(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=(4, 4), norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.dim_scale = dim_scale
        self.expand = nn.Linear(dim, dim_scale[0] * dim_scale[1] * dim, bias=False)
        self.output_dim = dim
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 shift_size=(0, 0) if (i % 2 == 0) else [x // 2 for x in window_size],
                                 mlp_ratio=mlp_ratio,
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=(4, 4), in_chans=1,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                layer_up = PatchExpand(
                    ...
                    dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)), dim_scale=2, norm_layer=nn.LayerNorm)
            else:
        ...
        self.phase_projector = nn.Hardtanh(0, 2 * torch.pi)
    ...
    def up_x4(self, x):
        ...
        if self.final_upsample == "expand_first":
            ...
            # x = torch.atan2(x[:, 0:1, :, :], x[:, 1:2, :, :])
            # x = self.phase_projector(x)
