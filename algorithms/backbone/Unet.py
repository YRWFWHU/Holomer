...
class UpBlock3D(nn.Module):
    ...
    def forward(self, x, skipped=None):
        if skipped is not None:
            output = torch.cat([skipped, x], dim=1)
        else:
            output = x
...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        self.in_layer += [nn.BatchNorm2d(nf0, affine=True)]
        self.in_layer += [nn.LeakyReLU(0.2, True)]
        ...
        if not outermost_linear:
            self.out_layer += [nn.BatchNorm2d(out_channels, affine=True)]
            self.out_layer += [nn.ReLU(True)]
        ...
        self.phase_projector = nn.Hardtanh(-math.pi, math.pi)

    def forward(self, x):
        in_layer = self.in_layer(x)
        unet = self.unet_block(in_layer)
        out_layer = self.out_layer(unet)
        return self.phase_projector(out_layer)
